...
class PointResourceIT {
    ...
    /**
     * This repository is mocked in the com.zero.repository.search test package.
     *
     * @see com.zero.repository.search.PointSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void putNewPoint() throws Exception {
        ...
        // Validate the Point in Elasticsearch
        verify(mockPointSearchRepository).save(testPoint);
    }
    ...
    void searchPoint() throws Exception {
        ...
        // Search the point
        restPointMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + point.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(point.getId().intValue())))
            .andExpect(jsonPath("$.[*].title").value(hasItem(DEFAULT_TITLE)))
